...
class Ver_pub extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('Persona/Ver_pub/Ver_pub',$view);
	}
	...
	public function updatePub(){
		...
		$id_pub = $this->input->post('Upid_pubF');
		$dataForm = array(
			'titulo' => $this->input->post('Uptitulo'),
			'descripcion' => $this->input->post('Updescripcion'),
			'direccion' => $this->input->post('Upubicacion'),
			'tiempo_disponible' => $this->input->post('Uptiempo'),
			'estado'=> 'activo'
		);
		$datos_json= array();
	...
	public function rePublicar(){
		...
		$id_pub = $this->input->post('Repid_pubF');
		$dataForm = array(
			'titulo' => $this->input->post('Reptitulo'),
			'descripcion' => $this->input->post('Repdescripcion'),
			'direccion' => $this->input->post('Repubicacion'),
			'tiempo_disponible' => $this->input->post('Reptiempo'),
			'estado'=> 'activo',
			'fecha' => $fecha_actual
		);
		$datos_json= array();
	...
	public function changeImagePub(){
		$this->load->model('Publicaciones/Publicaciones_model');
		...
		$id_pub = $this->input->post('Upid_pubIm');
		$dataForm = array(
			'imagen' => $datename."img_thumb.jpg"
		);
		$datos_json= array();

			if (!$this->upload->do_upload('UpimagenPub') ){
				$datos_json["estado"] = "ErrorUpload";
				$datos_json["error"] = $this->upload->display_errors();
			}else{
				...
				$data = array('upload_data' => $this->upload->data());
          		...
          		if ($this->Publicaciones_model->updatePub($id_pub,$dataForm)) {
          			$datos_json["estado"] = "Success";
          			$datos_json['img'] = $datename."img_thumb.jpg";  	
				}else{
					$datos_json["estado"] = "ErrorPublic";
					unlink('assets/files/publicaciones/'.$datename."img_thumb.jpg");
				}
	...
	public function getPub(){
		...
		$numPub = $this->Publicaciones_model->getNumPublicByUser($usuario);
		...
		$data ['pub'] = $this->Publicaciones_model->getPublicByUser($limit,$pubpaginaAct,$usuario);
		$data ['pag'] = $lista;
	...
	public function eliminarPub(){
		...
		if ($this->Publicaciones_model->udatePubState($id_pub,$data)) {
			$answer = array(
				'answer' => 'OK'
			);
		}else{
